...
- **Options dropdown** in the header (before Clear), persisted in the BepInEx config: *Show Δv column* (default off — column hidden) and *Show next transfer window* (default off — hides the dimmed second row **and skips its computation**, roughly halving refresh cost; re-enabling backfills via cheap partial recalcs).
