...
- **Alert lead time**: new "Alert [N] day(s) before" editable field in the Options dropdown (default 0, persisted). Alarms are now day-precise — the key stores the departure day — and fire N days ahead; legacy month-granularity alarms from old sidecars still fire on their month. Notification/toast text is capitalized ("LAUNCH WINDOW (Optimal) …") and dates use yy/MM/dd.
